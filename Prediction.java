...
public class Prediction 
{
	 ...
	 */
	public static int[] talking_Tiles(int Tile_id)
	{
	 ...
	 */
	public static double score_Tile(int Tile_id)
	{
		try
		{
			final double[] tile_weights = {0.0,0.2,0.07,0.17,0.2};	//weight constants on how impactful the amount of Tiles is
					
			int[] talking_tile = talking_Tiles(Tile_id);			//amount of talking tiles around given Tile_id
	 ...
	 * @return An ArrayList of Arrays that represents impossible - unlikely shapes
	 */
	public static HashSet<int[]> unlikely_Tiles(ArrayList<Tile> discard_Tiles)
	{
		/*
		 * This segment of the algorithm searches for completed Groups with given Tile
		 */
		interface IntTo2DArrayFunction {ArrayList<int[]> apply(int value);}
		IntTo2DArrayFunction generate2DArray = (int Tile_id) -> 
		{
			if(Tile_id < 0 || Tile_id > 33) return null;
		...
		for(Tile input_tile: discard_Tiles) 
			for(int[] shapes: generate2DArray.apply(input_tile.getTileID()))
	 ...
	 */
	public static ArrayList<ArrayList<int[]>> common_waits(int discard_TileID) throws IllegalArgumentException
	{
	...
	public static ArrayList<int[]> all_possible_waits(int readied_TileID, HashSet<int[]> unlikely_shapes)
	{
		try
		{
			ArrayList<int[]> return_data = new ArrayList<int[]>();
			for(ArrayList<int[]> categorized_shapes : common_waits(readied_TileID))
	 ...
	 */
	public static class HiddenHand_search
	{
		/**
		 ...
		 */
		public HiddenHand_search(Mahjong_hand given_hand)
		{
		 ...
		 */
		public HiddenHand_search(HiddenHand_search clone)
		{
	 ...
	 */
	public static class Early_search extends HiddenHand_search
	{	
		/**
		 * Minimum constructor requires a valid Mahjong_hand
		 * @param given_hand A hand that can be searched through
		 */
		public Early_search(Mahjong_hand given_hand)
		{
		 ...
		 */
		public Early_search(Early_search clone)
		{
			this.probability_map_ = new ArrayList<Double>(clone.getProbability_map());
	 ...
	 */ 
	public static class MiddleGame_search extends HiddenHand_search
	{
		/**
		 * Minimum constructor requires a valid Mahjong_hand
		 * @param given_hand
		 */
		public MiddleGame_search(Mahjong_hand given_hand)
		{
		 ...
		 * @param clone Another instance of Early_search object
		 */
		public MiddleGame_search(Early_search clone)
		{
	 ...
	 */
	public static class LateGame_search extends HiddenHand_search
	{
	...
	{
		ArrayList<Tile> example_discard = new ArrayList<Tile>();
		example_discard.add(new Tile(0));
		example_discard.add(new Tile(5));
		for(int[] shape: unlikely_Tiles(example_discard))
		{
			System.out.print("(");
			for(int i = 0; i < shape.length; i++)
			{
				if(i + 1 == shape.length) System.out.print(shape[i]);
				else System.out.print(shape[i] + ", ");
			}
			System.out.println(")");
		}
		...
		for(int[] shape: MJ_Hand_tools.allPossibleSeqShape(0))
		{
			System.out.print("(");
			for(int i = 0; i < shape.length; i++)
			{
